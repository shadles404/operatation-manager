...
export const LocalBudgetView: React.FC = () => {
  ...
  // Get real database budget summary and category breakdown for Local pool in selected month
  const summary = store.getBudgetSummary('Local', selectedMonth);
  const categoryBreakdown = store.getCategoryBreakdown('Local', selectedMonth);
  const availableCategories = store.getCategories('Local', selectedMonth);

  // Filter actual DB expenses for Local pool and selected month
  const monthExpenses = store.getExpenses().filter(
    e => e.budgetType === 'Local' && toMonthKey(e.date) === selectedMonth
  );
  ...
  const handleSaveBudget = async (e: React.FormEvent) => {
    ...
    setBudgetError(null);
    const res = await store.setMonthlyBudget('Local', selectedMonth, num);
  ...
  const handleOpenLogExpense = () => {
    ...
    setExpenseDesc('');
    setExpenseAmount('');
    // Default date to today, or if today is in a different month, default to 1st of selectedMonth
  ...
  const handleSaveExpense = async (e: React.FormEvent) => {
    ...
    if (!res.success) {
      ...
      // If the expense was recorded for a different month than currently selected, switch to that month
  ...
  const getCategorySubtitle = (cat: string) => {
    switch (cat) {
      case 'Influencers':
        return 'Local influencer collaborations, activations & sponsorships';
      case 'Billboards':
        return 'Local outdoor static billboard displays & roadside hoardings';
      case 'LCD Screens':
        return 'Local commercial LCD screen networks & video broadcasts';
      default:
        return 'Local marketing collateral, events, POS & miscellaneous expenses';
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-xl p-5">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white tracking-tight">Local Marketing Budget</h1>
              <p className="text-sm text-slate-400">
                Track and manage local marketing expenses (Influencers, Billboards, LCD Screens, Other)
              </p>
            </div>
          </div>
        </div>
          ...
          {canManage && (
            <>
              <button
                ...
                className="flex items-center gap-2 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-medium transition-colors shadow-sm cursor-pointer"
              >
      ...
      {summary.totalBudget > 0 && summary.remaining < 0 && (
        <div className="flex items-start gap-3 bg-rose-500/10 border border-rose-500/30 rounded-xl p-4 text-rose-300">
          ...
          <div className="text-sm">
            <span className="font-semibold text-rose-200">Budget Limit Exceeded:</span> Total recorded local expenses ({formatCurrency(summary.spent)}) have exceeded the configured monthly budget of {formatCurrency(summary.totalBudget)} for {toMonthDisplay(selectedMonth)} by {formatCurrency(Math.abs(summary.remaining))}.
          </div>
        </div>
      )}
            ...
            <div className="w-full bg-slate-800 rounded-full h-1.5 mt-3 overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-300 ${
                  ...
                  summary.utilization >= 80 ? 'bg-amber-500' : 'bg-emerald-500'
                }`}
      ...
      <div className="space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div>
            <h2 className="text-base font-semibold text-white">Category Breakdown for {toMonthDisplay(selectedMonth)}</h2>
            <span className="text-xs text-slate-400">Separated by real database records & target allocations</span>
          </div>
          {canManage && (
            <button
              onClick={() => setIsCategoryModalOpen(true)}
              className="self-start sm:self-auto text-xs text-indigo-400 hover:text-indigo-300 font-medium flex items-center gap-1.5 transition-colors cursor-pointer"
            >
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-xl overflow-hidden">
        <div className="p-5 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-white">
              Recorded Local Expenses ({toMonthDisplay(selectedMonth)})
            </h2>
          ...
          {canAddExpense && (
            <button
              onClick={handleOpenLogExpense}
              className="flex items-center gap-1.5 text-xs text-indigo-400 hover:text-indigo-300 font-medium self-start sm:self-auto transition-colors"
            >
        ...
        {monthExpenses.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h3 className="text-sm font-semibold text-white">No Local Expenses Recorded</h3>
            <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1">
              There are no expenses recorded for Local Budget in {toMonthDisplay(selectedMonth)}. All budget calculations reflect live database records.
            </p>
            {canAddExpense && (
              <button
                onClick={handleOpenLogExpense}
                className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-medium transition-colors"
              >
                <Plus className="w-3.5 h-3.5" />
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-6 shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-white text-base">Set Local Monthly Budget</h3>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-8 pr-4 py-2 text-white text-sm focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <p className="text-xs text-slate-400 mt-1.5">
                  This value is saved in Firestore and sets the total allowed local marketing spend for {toMonthDisplay(selectedMonth)}.
                </p>
              </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-lg w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-white text-base">Log Local Marketing Expense</h3>
                  <p className="text-xs text-slate-400">Record a real expenditure against Local Budget</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleSaveExpense} className="mt-5 space-y-4">
              ...
              {/* Budget Pool (Fixed to Local Budget) */}
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5">
                  Budget Pool *
                </label>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Local Billboard Q3 placement or Influencer campaign"
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3.5 py-2 text-white text-sm focus:outline-none focus:border-indigo-500"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Invoice number, vendor name, or additional campaign details..."
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3.5 py-2 text-white text-sm focus:outline-none focus:border-indigo-500"
                />
              </div>
