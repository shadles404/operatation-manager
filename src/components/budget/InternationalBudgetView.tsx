...
import { Globe, Wallet, Plus, Edit2, Trash2, X, AlertTriangle, Check } from 'lucide-react';
import { store } from '../../services/store';
import { Budget } from '../../types';

export const InternationalBudgetView: React.FC = () => {
  const intlBudgets = store.getBudgets().filter(b => b.budgetType === 'International');

  const totalAllocated = intlBudgets.reduce((sum, b) => sum + b.allocated, 0);
  const totalSpent = intlBudgets.reduce((sum, b) => sum + b.spent, 0);
  const totalCommitted = intlBudgets.reduce((sum, b) => sum + b.committed, 0);
  const totalRemaining = intlBudgets.reduce((sum, b) => sum + b.remaining, 0);
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    // Check duplicate
    const exists = intlBudgets.some(b => b.category.toLowerCase() === formCategory.trim().toLowerCase());
    if (exists) {
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            <span>International Marketing Budget Pools</span>
          </h1>
          <p className="text-xs text-slate-400 mt-0.5">Global digital campaigns, international influencer partnerships, & trade expos</p>
        </div>

        {canManage && (
          <button
            onClick={handleOpenAdd}
            className="px-4 py-2 bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold text-xs rounded-xl shadow-lg shadow-sky-500/20 flex items-center gap-2 transition-all cursor-pointer shrink-0"
          >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 border-b border-slate-800 font-bold text-white text-sm">
          International Category Allocations ({intlBudgets[0]?.period || 'August 2026'})
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl relative">
            ...
            <div className="flex items-center gap-2 text-sky-400 mb-2">
              ...
              <h3 className="text-base font-bold text-white">Create International Budget Pool</h3>
            </div>
            <p className="text-xs text-slate-400 mb-4">Establish a new international category budget allocation pool.</p>
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4 text-xs text-slate-300">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Meta Ads Global Campaign"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl relative">
            ...
            <div className="flex items-center gap-2 text-sky-400 mb-2">
              ...
              <h3 className="text-base font-bold text-white">Modify International Budget Pool</h3>
            </div>
            <p className="text-xs text-slate-400 mb-4">Set the allocated budget or alter the category name for this international operation pool.</p>
