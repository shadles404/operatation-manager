...
export const LCDPaymentsView: React.FC = () => {
  const [permissionError, setPermissionError] = useState<string | null>(null);
  const payments = store.getPayments().filter(p => p.paymentType === 'LCD Screen');
  const canApprove = store.hasPermission('lcd_payments', 'approve');
  ...
  const handleUpdateStatus = (payment: CentralPayment, nextStatus: CentralPayment['status']) => {
    if ((nextStatus === 'Approved' || nextStatus === 'Paid') && !canApprove) {
      setPermissionError('Permission denied: Approval required for LCD screen payments');
      return;
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            <span>LCD Screen Rent & Maintenance Payments</span>
          </h1>
          <p className="text-xs text-slate-400 mt-0.5">Track venue monthly leases and technical maintenance for LCD screens</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-300">
              {payments.map(pay => (
                <tr key={pay.id} className="hover:bg-slate-800/40 transition-colors">
                  <td className="py-3.5 px-4 font-mono font-bold text-purple-400">{pay.paymentId}</td>
